...
public class TextSpace extends HBox {
	 ...
	 */
	public TextSpace() {
		final FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/textspace.fxml"));
	...
	public void initialize() {
		...
		// add highlighter
		extraSelection = new SelectionImpl<>(I18N.getString("button.otherselect.text"), textArea, path -> {
			// make rendered selection path look like a yellow highlighter
			path.setStrokeWidth(0);
			path.setFill(Color.YELLOW);
		});
		if (!textArea.addSelection(extraSelection)) {
			throw new IllegalStateException(I18N.getString("button.addzone.text"));
		}
	}
	...
	private void textAreaChangeListener() {
		textArea.textProperty().addListener(
				(observable, oldValue, newValue) -> mediator.getEventBuilder().withEvent(Events.TEXT_CHANGED).build());
	}
	...
	public void selectText(final String str) {
		...
		extraSelection.selectRange(startIndices.get(startIndicesTracker),
				startIndices.get(startIndicesTracker) + str.length());
	}
